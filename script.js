...
const placename = async (lat1 = 12, lng1 = 77) => {
    try {
        const response = await axios.post(
            `${CONFIG.API_ENDPOINTS.GEOCODING}?latlng=${lat1},${lng1}&key=${window.GOOGLE_MAPS_API_KEY}`
        );
...
const aqi = async (lat1 = 12, lng1 = 77) => {
    try {
        const response = await axios.post(
            `${CONFIG.API_ENDPOINTS.AIR_QUALITY}?key=${window.GOOGLE_MAPS_API_KEY}`,
            {
                location: {
                  latitude: lat1,
                  longitude: lng1,
...
const gettime=async (lat1 = 12, lng1 = 77)=>{
    ...
    try {
        const response = await axios.post(`${CONFIG.API_ENDPOINTS.TIMEZONE}?location=${lat1}%2C${lng1}&timestamp=${now1}&key=${window.GOOGLE_MAPS_API_KEY}`);
    ...
    if(!nextoken && STATE.count>1){
        ...
        try {
            response = await axios.post(
                    `https://airquality.googleapis.com/v1/history:lookup?key=${window.GOOGLE_MAPS_API_KEY}`,
                {
                    location: {
                        latitude: lat1,
                        longitude: lng1,
